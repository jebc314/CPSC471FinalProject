...
def segmentation_pipeline(images):
    ...
    for img in all_images_segmentation:
        ...
        mask = torch.eq(segmentation_id, (count // 10 + 1)).int()
        temp = transform(torch.stack([mask, mask, mask]))[0]
        values = temp.unique().tolist()
        # sometimes there are no 0's
        if values[0] != 0: values.insert(0, 0)
...
torch.save(ground_truth_masks, "ImageNet/ground_truth_masks.pt")